...
description: 操作和维护 Ezviz Office 本地后台及 EZVIZ 国际商城后台。用户要求查找后台导航、启动本地工具、登录或切换国家站点、配置或巡查 Banner/Popup/UTM、提交 TDK、配置或恢复 WTB、上传语言包、处理产品资料、批量读取产品 Detail、批量替换 Detail 图片或链接地址、精确删除 Detail 代码块、执行官网巡查、连接或排查 GA4/GSC/Search Console，或扩展相关自动化脚本时使用。
---
...
4. 从 `GET /api/campaign/sites` 读取启用站点。真实提交一次只选择一个站点，并核对登录后的账号身份。
...
- 连接、验证或迁移 GA4 / Google Search Console：读取 [references/google-ga4-gsc.md](references/google-ga4-gsc.md)。只报告凭据文件是否存在，不打印 OAuth client、refresh token 或其他密钥内容。

## 执行原则

1. 先调用 `plan`、`preview`、只读接口或页面上的“生成清单”按钮。
2. 校验站点、产品名、文件、时间、链接和平台名称。
3. 登录并核对目标账号；账号不匹配时先退出或清理专用会话，再重新登录。
...
- 不把账号密码、Cookie、Storage State、OAuth client、refresh token 或账号 Excel 加入 Git。
