...
| 分组 | 页面入口 | 主要能力 | 默认风险 |
| --- | --- | --- | --- |
| 基础工具 | 一键内联打包 | 将页面 CSS、图片和 HTML 处理为带 Webflow 页面作用域的上线代码；打包后下载 `store.html`，同时在页面展示最终 CSS 并提供 `webflow.scoped.css` 下载；不处理语言字段 | 本地生成；图片模式可能上传文件 |
| 基础工具 | 图片 AI 标志 | 支持拖拽或文件选择多张图片，用 Canvas 在右下角叠加 `AI GENERATED` 标志，并在列表中逐张导出 PNG | 本地生成；不上传、不覆盖原文件 |
...
| 基础工具 | i18n 语言转换 | 英文文案先按单产品语言包原文复用 Key，未命中再按产品名生成；总语言包回查 HTML 已有 Key；页面和 Excel 分表输出三类字段 | 本地处理 |
...
| 临时功能 | Detail 批量替换/删除 | 七列表格逐产品配置 Detail 地址、代码块和 Product Album 本地高清图路径；提交时上传并覆盖 | `preview` 只读；`submit` 每产品保存一次并逐项回读 |
