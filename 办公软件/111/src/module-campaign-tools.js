(function () {
  ...
  async function inspectDtcAssets() {
    el.dtcInspectAssets.disabled = true;
    setStatus("正在读取 DTC 本地素材并检查国家目录、文件命名和大小...");
    revealOutput();
    try {
      const payload = await postForm("/api/campaign/dtc-assets", dtcFormData(false));
      writeOutput(payload.inspection);
      const skipped = payload.inspection.skippedCount || 0;
      setStatus(skipped
        ? `DTC 素材检查完成：可执行 ${payload.inspection.readyCount || 0} 个国家，跳过 ${skipped} 个，请查看原因。`
        : "DTC 素材检查通过：5 个国家均可执行。", skipped ? "warn" : "ok");
      revealOutput();
    } catch (error) {
      setStatus("DTC 素材读取失败：" + (error.message || error), "warn");
      writeOutput("DTC 素材读取失败：\n" + (error.message || error));
      revealOutput();
    } finally {
      el.dtcInspectAssets.disabled = false;
    }
  }
