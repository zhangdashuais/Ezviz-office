...
bullets(doc, ["后台产品一键替换仍为开发中入口，暂不执行真实操作。", "部分 Banner / Popup 巡查依赖 E:\\Website-backend\\backend-operations 下的配置和脚本。", "SharePoint 资料处理目前以生成归档计划为主。", "后台页面结构或接口变化可能导致自动化控件无法匹配，需要根据日志更新规则。"])
